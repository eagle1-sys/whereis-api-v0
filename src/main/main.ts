...
const apiCmd = new Deno.Command(Deno.execPath(), {
  args: ["serve", "--parallel", "--port", port, "--allow-sys", "--allow-net", "--allow-env", "--allow-read","--allow-write", "--allow-ffi", "src/main/api.ts"],
  stdout: "inherit",
  stderr: "inherit",
}).spawn();

const schedulerCmd = new Deno.Command(Deno.execPath(), {
  args: ["run", "--allow-sys", "--allow-env", "--allow-read", "--allow-net", "--allow-write", "--allow-ffi", "src/main/schedule.ts"],
  stdout: "inherit",
  stderr: "inherit",
}).spawn();
